...
"""FastWAM model wrapper for joint video and action modeling."""
...
class FastWAM(torch.nn.Module):
    ...
    def __init__(
        ...
        """Initialize FastWAM with video, action, MoT, and scheduler components."""
        super().__init__()
    ...
    def from_wan22_pretrained(
        ...
        """Build a FastWAM instance from Wan2.2 pretrained components."""
    ...
    def to(self, *args, **kwargs):
        """Move FastWAM modules to a device or dtype."""
    ...
    def _check_resize_height_width(height, width, num_frames):
        """Round video dimensions to valid VAE and temporal sizes."""
        if height % 16 != 0:
    ...
    def encode_prompt(self, prompt: Union[str, Sequence[str]]):
        """Encode text prompts into context embeddings and masks."""
    ...
    def _append_proprio_to_context(
        ...
        """Append encoded proprioception as an extra context token."""
    ...
    def _encode_video_latents(self, video_tensor, tiled=False, tile_size=(30, 52), tile_stride=(15, 26)):
        """Encode a video tensor into VAE latent space."""
        z = self.vae.encode(
            video_tensor,
            device=self.device,
            tiled=tiled,
    ...
    def _encode_input_image_latents_tensor(
        ...
        """Encode a single conditioning image into VAE latent space."""
    ...
    def _decode_latents(self, latents, tiled=False, tile_size=(30, 52), tile_stride=(15, 26)):
        """Decode video latents into PIL image frames."""
    ...
    def build_inputs(self, sample, tiled: bool = False):
        """Prepare training inputs, noisy latents, targets, and masks."""
        video = sample["video"]
        ...
        if action_horizon % (num_frames - 1) != 0:
            raise ValueError(
                ...
                f"video transitions ({num_frames - 1}), got {action_horizon}"
            )
    ...
    def _compute_video_loss_per_sample(
        ...
        """Compute per-sample video reconstruction loss with optional padding masks."""
    ...
    def _predict_joint_noise(
        ...
        """Predict video and action noise jointly through the MoT model."""
        video_pre = self.video_expert.pre_dit(
            x=latents_video,
            timestep=timestep_video,
            context=context,
    ...
    def _predict_action_noise(
        ...
        """Predict action noise conditioned on the first video frame."""
    ...
    def _predict_action_noise_with_cache(
        ...
        """Predict action noise using a prefetched video key-value cache."""
        action_pre = self.action_expert.pre_dit(
            action_tokens=latents_action,
            timestep=timestep_action,
            context=context,
    ...
    def infer_joint(
        ...
        """Infer video frames and action tokens from an image and prompt/context."""
        self.eval()
        ...
        if action is not None:
            ...
            if action.ndim != 3 or action.shape[0] != 1 or action.shape[1] != action_horizon:
                # NOTE: This enforces action condition to have the same shape as action horizon to predict.
                raise ValueError(
                    "`action` must have shape [1, T, a_dim] or [T, a_dim], got "
                    f"{tuple(action.shape)} with action_horizon={action_horizon}"
                )
    ...
    def infer_action(
        ...
        """Infer action tokens from an image and prompt/context."""
        self.eval()
    ...
    def infer(
        ...
        """Infer video frames using the joint inference path."""
    ...
    def load_checkpoint(self, path, optimizer=None):
        ...
        if self.proprio_encoder is not None:
            ...
            logger.warning(
                "Checkpoint contains `proprio_encoder` weights but current model has "
                "`proprio_dim=None`; ignoring."
            )
    ...
    def forward(self, *args, **kwargs):
        """Dispatch forward calls to the training loss computation."""
