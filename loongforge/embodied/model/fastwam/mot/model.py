...
"""Mixture-of-Transformers module for combining FastWAM experts."""
...
class MoT(nn.Module):
    """Mixture-of-Transformers wrapper that mixes video and action experts."""
    ...
    def _split_modulation(block, t_mod: torch.Tensor):
        """Split a block modulation tensor into attention and MLP controls."""
    ...
    def _apply_expert_post_block(
        ...
        """Apply expert-specific post-attention residual, cross-attn, and MLP."""
    ...
    def _apply_post_with_optional_checkpoint(
        ...
        def _post_fn(
            ...
            """Apply the captured expert post block inside checkpointing."""
    ...
    def forward(
        ...
        """Run all MoT layers over video/action expert token dictionaries."""
        ...
        for layer_idx in range(self.num_layers):
            ...
            for name, seq_len in zip(self.expert_order, seq_lens):
                # split mixed attention output and apply post-attention blocks for each expert
                end = start + seq_len
