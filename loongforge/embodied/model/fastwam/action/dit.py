...
"""Action DiT expert used by FastWAM models."""
...
class ActionHead(nn.Module):
    """Projection head that maps hidden action tokens back to action dimensions."""

    def __init__(self, hidden_dim: int, out_dim: int, eps: float):
        """Initialize normalization, projection, and timestep modulation."""
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        shift, scale = (modulation + t.unsqueeze(1)).chunk(2, dim=1)
...
class ActionDiT(nn.Module):
    """Diffusion Transformer expert for action-token denoising."""
    ...
    def __init__(
        ...
        with torch.random.fork_rng():
            ...
            nn.init.normal_(self.action_encoder.weight, mean=0, std=0.02)
    ...
    def from_pretrained(
        ...
        """Create an ActionDiT and optionally load a preprocessed action backbone."""
        ...
        if policy:
            logger.info("ActionDiT backbone payload policy: %s", policy)
        ...
        cls._validate_backbone_meta(meta, expected_meta, action_dit_pretrained_path)
    ...
    @classmethod
    def _validate_backbone_meta(
        cls,
        meta: dict[str, Any],
        expected_meta: dict[str, Any],
        action_dit_pretrained_path: str,
    ) -> None:
        """Validate metadata stored alongside a preprocessed action backbone."""
    ...
    def post_dit(self, tokens: torch.Tensor, pre_state: Dict[str, Any]) -> torch.Tensor:
        """Project DiT hidden tokens back to action-token space."""
        del pre_state
    ...
    def forward(
        ...
        """Run action-token denoising through the DiT blocks."""
